...
if __name__ == '__main__':
    ...
    reward_manager.add_location_event("door", reward=0.4)
    reward_manager.add_kill_event("demon", reward=0.6)
    reward_manager.add_kill_event("lava", reward=0.5)
    reward_manager.add_location_event("stairs", reward=0.8)
    ...
    fname = str(env) + '_num-steps_'
    #pixel_obs = "pixel"
    env = RenderRGB(env, "pixel")
    env = gym.wrappers.RecordVideo(env, "recordings", episode_trigger= lambda x: x % 2 == 0 )
    ...
    hyper_params = {
            ...
            "learning-rate": 0.01,  # learning rate for RMSprop optimizer
            "discount-factor": 0.99,  # discount factor
            "num-steps": int(1e6),  # total number of steps to run the environment for
            "batch-size": 32,  # number of transitions to optimize at the same time 32
